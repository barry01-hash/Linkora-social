...
function mapPrefsToPayload(prefs: PreferencesState): NotificationPreferencePayload {
  return {
    follow_notifications: prefs.newFollowers,
    like_notifications: prefs.likes,
    tip_notifications: prefs.poolTipReceived,
    pool_notifications: prefs.poolTipReceived,
    governance_notifications: prefs.governanceUpdates,
    moderation_notifications: true,
    post_notifications: prefs.commentsReplies,
  };
}
...
export const NotificationPreferences: React.FC = () => {
      ...
      if (!preferences.masterPushEnabled) {
        ...
        if ("Notification" in window) {
          ...
          if (permission !== "granted") {
            setMessage({
              type: "error",
              text: "Permission for notifications was denied by the browser.",
            });
            setLoading(false);
            return;
          }
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      if (address) {
        await savePreferencesToBackend(address, mapPrefsToPayload(preferences), subscription);
      }
  ...
  return (
    <div className="max-w-md mx-auto bg-white dark:bg-gray-800 shadow-md rounded-lg p-6 mt-6 border border-gray-200 dark:border-gray-700">
      ...
      <div className="flex items-center justify-between pb-4 mb-4 border-b border-gray-200 dark:border-gray-700">
        <div>
          ...
          <p className="text-xs text-gray-500">
            Required to receive native alerts on your operating system
          </p>
        </div>
        <input
          type="checkbox"
      ...
      <div
        className={`space-y-4 ${!preferences.masterPushEnabled ? "opacity-50 pointer-events-none" : ""}`}
      >
